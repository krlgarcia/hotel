...
class guestregistration(models.Model):
    ...
    def action_reserve(self):
        for rec in self:
            if not (rec.guest_id):
                raise ValidationError('Please add a valid guest.')
            elif not (rec.roomname):
                raise ValidationError('Please add a valid room number.')
            elif not (rec.datefromSched):
                raise ValidationError('Please add a valid date from schedule.')
            elif not (rec.datetoSched):
                raise ValidationError('Please add a valid date to schedule.')
            elif (rec.datetoSched < rec.datefromSched):
                raise ValidationError('Invalid range.')
            else:
                pkid = rec.id
                self.env.cr.execute("select * from public.fncheck_registrationconflict(" + str(pkid) + ")")
